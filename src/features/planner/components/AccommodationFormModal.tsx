...
const AccommodationFormModal: React.FC<Props> = ({
                                                   isOpen,
                                                   onDismiss,
                                                   tripId,
                                                   accommodation,
  ...
  const dayStops = useLiveQuery(async () => {
    ...
    const day = await db.days
        .where('tripId')
        .equals(tripId)
        .filter(d => d.date === checkIn)
        .first()
  ...
  useEffect(() => {
    ...
    if (accommodation) {
      ...
      const nextDay = initialDate
          ? new Date(
              new Date(initialDate + 'T00:00:00Z').getTime() + 86400000
  ...
  async function handleSave() {
    ...
    // Remember the currency for the next new expense/transport/accommodation.
    if (priceCurrency) {
    ...
    // Close first, then persist in the background.
    onDismiss()

    try {
      if (existing) {
        await AccommodationRepository.update(
            existing.id,
            updates,
            sel
        )
