import type { Expense, Trip } from '../../../db/schema'
import {
...
const BudgetBar: React.FC<Props> = ({
                                      trip,
                                      totalSpent,
                                      expenses,
                                    }) => {
  ...
  return (
      <div style={{ padding: '0.75rem 1rem' }}>
        {trip.budget.dailyAmount && (() => {
          ...
          const cumulative =
              trip.budget.dailyAmount * daysElapsed

          const pct = Math.min(
              100,
              (spentTillToday / cumulative) * 100
          )
          ...
          return (
              <div style={{ marginBottom: 8 }}>
                ...
                {spentTillToday.toFixed(0)} / {cumulative.toFixed(0)}{' '}
        ...
        {trip.budget.total && (() => {
          const status = getTotalBudgetStatus(
              totalSpent,
              trip.budget.total
          )
          ...
          const pct = Math.min(
              100,
              (totalSpent / trip.budget.total) * 100
          )
          ...
          return (
              <div>
                ...
                >
                  ...
                  <span style={{ color: STATUS_COLORS[status] }}>
                {totalSpent.toFixed(0)} / {trip.budget.total}{' '}
                    {trip.defaultCurrency}
              </span>
                </div>
